...
import { CAPITALS_QUIZ_QUESTIONS, QuizQuestion, CAPITALS_PRIZES, LocalizedString } from './capitalsData';

type Props = {
    onClose: () => void;
};

const CapitalsQuizPage: React.FC<Props> = ({ onClose }) => {
    ...
    // Use a ref to store the shuffled question set for the current level to avoid re-shuffling on render
    const [levelQuestions, setLevelQuestions] = useState<QuizQuestion[]>([]);

    useEffect(() => {
        if (gameState === 'playing') {
            const targetLevel = currentLevel + 1;
            // Find all questions for this level
            const pool = CAPITALS_QUIZ_QUESTIONS.filter(q => q.level === targetLevel);
            
            if (pool.length > 0) {
                const randomIndex = Math.floor(Math.random() * pool.length);
                // Since the answers are already shuffled in the data generator, 
                // we just need to pick a random question for this level.
                setCurrentQuestion(pool[randomIndex]);
                setShowOptions(false); // Reset for new question
            } else {
                console.error(`No questions found for level ${targetLevel}`);
                setCurrentQuestion(CAPITALS_QUIZ_QUESTIONS[0]);
    ...
    const handleAnswer = (index: number) => {
        ...
        setTimeout(() => {
            if (index === currentQuestion.correctIndex) {
                if (currentLevel === CAPITALS_PRIZES.length - 1) {
                    setGameState('won');
                } else {
    ...
    const handleFiftyFifty = () => {
        ...
        setShowOptions(true); // Reveal options if hidden
    };
    ...
    const getLocStr = (ls: LocalizedString) => {
        return ls[lang] || ls.en;
    }

    const formatQuestion = (q: QuizQuestion) => {
        const template = getLocStr(q.question);
        const country = getLocStr(q.countryName);
        return template.replace('{country}', country);
    };

    const getShowOptionsText = () => {
         switch(lang) {
            case 'de': return "Antworten zeigen";
            case 'es': return "Mostrar opciones";
            default: return "Show Options";
        }
    };
    ...
    if (gameState === 'intro') {
        return (
            <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-teal-900 to-gray-900 z-0" />
                <div className="z-10 text-center max-w-md w-full">
                    <h1 className="text-5xl font-black mb-2 tracking-tighter">
                        <span className="text-white">CAPITALS</span>
                        <span className="text-teal-500"> QUIZ</span>
                    </h1>
                    ...
                    <p className="text-gray-400 mb-8 text-lg">
                        15 Questions.<br/>
                        From Paris to Ngerulmud.<br/>
                        Rule the World.
                    </p>
                    <button 
                        onClick={() => setGameState('playing')}
                        className="w-full bg-teal-600 hover:bg-teal-700 text-white font-bold py-4 rounded-xl text-xl transition-transform transform active:scale-95 shadow-lg mb-4"
                    >
                        START GAME
                    </button>
        ...
        return (
            <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-6 text-center">
                <h2 className="text-4xl font-bold mb-4">
                    {gameState === 'won' ? 'WORLD RULER!' : 'LOST IN TRANSIT'}
                </h2>
                <p className="text-xl text-gray-300 mb-2">
                    You reached level {currentLevel + 1}
                </p>
                <p className="text-teal-500 font-bold text-2xl mb-8">
                    {CAPITALS_PRIZES[Math.max(0, currentLevel - (gameState === 'lost' ? 1 : 0))]}
                </p>
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white flex flex-col">
            <div className="flex justify-between items-center p-4 bg-gray-800/50 backdrop-blur-sm">
                <span className="font-bold text-lg text-teal-400">Lv. {currentLevel + 1} / 15</span>
            ...
            <div className="flex-grow flex flex-col md:flex-row max-w-6xl mx-auto w-full p-4 gap-6">
                <div className="md:order-2 md:w-64 flex-shrink-0 flex flex-col-reverse gap-1 p-4 bg-black/20 rounded-xl border border-white/5 overflow-y-auto max-h-[30vh] md:max-h-none">
                    {CAPITALS_PRIZES.map((prize, index) => {
                        let stateClass = "text-gray-600 bg-gray-800/50"; 
                        if (index < currentLevel) stateClass = "text-teal-300 font-bold bg-teal-900/20 border border-teal-500/30";
                        if (index === currentLevel) stateClass = "text-white font-bold bg-teal-600 shadow-[0_0_15px_rgba(20,184,166,0.5)] scale-105 z-10";
                ...
                <div className="flex-grow flex flex-col justify-center md:order-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-h-[200px] content-center">
                        {!showOptions ? (
                            <div className="col-span-1 md:col-span-2 flex justify-center items-center">
                                <button 
                                    onClick={() => setShowOptions(true)}
                                    className="py-4 px-8 rounded-xl text-xl font-bold bg-teal-600 hover:bg-teal-500 text-white shadow-lg transition-all transform hover:scale-105 animate-pulse"
                                >
                                    {getShowOptionsText()}
                                </button>
                            </div>
                            ...
                            currentQuestion.answers.map((answer, index) => {
                                ...
                                let btnClass = "bg-gray-800 border-2 border-gray-700 hover:border-gray-500 hover:bg-gray-700";
                                ...
                                if (isEliminated) {
                                    ...
                                    btnClass = "bg-teal-600 border-white";
                                }
                    ...
                    <div className="flex justify-center gap-4 mt-8">
                        <button 
                            ...
                            title="50/50"
                        >
                            <div className="w-14 h-14 rounded-full border-2 border-teal-500 flex items-center justify-center font-bold text-xl bg-gray-800">
                                50:50
                            </div>
                        </button>
                        ...
                        <button 
                            ...
                            title="Ask a Friend"
                        >
                            <div className="w-14 h-14 rounded-full border-2 border-teal-500 flex items-center justify-center bg-gray-800">
